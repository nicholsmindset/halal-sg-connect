import { BusinessImportData, businessImportSchema, csvHeaderMapping, ImportValidationResult, GeocodeResult } from '@/types/import';
...
export class CSVImportParser {
  ...
  static mapCSVHeaders(csvHeaders: string[]): Record<string, string> {
    ...
    for (const [standardField, possibleHeaders] of Object.entries(csvHeaderMapping)) {
  ...
  static transformCSVRow(row: any, headerMapping: Record<string, string>): Partial<BusinessImportData> {
    const transformed: any = {};
    
    for (const [csvHeader, value] of Object.entries(row)) {
      const standardField = headerMapping[csvHeader];
      if (standardField && value !== null && value !== undefined && value !== '') {
        // Handle array fields
        if (['features', 'tags', 'cuisine_types', 'amenities', 'special_diets', 'payment_methods', 'delivery_platforms'].includes(standardField)) {
          transformed[standardField] = typeof value === 'string' 
        ...
        else if (['rating', 'review_count', 'price_level'].includes(standardField)) {
  ...
  private static parseCoordinates(value: any): { lat: number; lng: number } | null {
  ...
  private static parseOpeningHours(value: any): Record<string, { open: string; close: string; closed?: boolean }> | null {
    if (typeof value !== 'string') return null;
    ...
    try {
      ...
      const simpleMatch = value.match(/(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})/);
      if (simpleMatch) {
...
export class ImportValidator {
  static validateBusinessData(data: any, rowNumber: number): ImportValidationResult {
    try {
...
export class DuplicateDetector {
  ...
  ): Promise<Array<{ business: Business; similarity: number; reasons: string[] }>> {
    const duplicates: Array<{ business: Business; similarity: number; reasons: string[] }> = [];
  ...
  private static calculateSimilarity(
    ...
    if (addressSimilarity > 0.7) {
      ...
      reasons.push(`Similar address (${Math.round(addressSimilarity * 100)}% match)`);
    }
...
export class GeocodingService {
  ...
  static async geocodeAddress(address: string): Promise<GeocodeResult> {
    try {
      ...
      if (lat < this.SINGAPORE_BOUNDS.south || lat > this.SINGAPORE_BOUNDS.north ||
          lng < this.SINGAPORE_BOUNDS.west || lng > this.SINGAPORE_BOUNDS.east) {
        return {
          success: false,
  ...
  private static mapToDistrict(postal: string): string {
    ...
    const sectorMapping: Record<number, string> = {
      1: 'Raffles Place', 2: 'Anson', 3: 'Queenstown', 4: 'Telok Blangah',
      5: 'Pasir Panjang', 6: 'High Street', 7: 'Middle Road', 8: 'Little India',
      9: 'Orchard', 10: 'Ardmore', 11: 'Newton', 12: 'Balestier',
      13: 'Macpherson', 14: 'Geylang', 15: 'Katong', 16: 'Bedok',
      17: 'Loyang', 18: 'Tampines', 19: 'Serangoon Garden', 20: 'Bishan',
      21: 'Upper Bukit Timah', 22: 'Jurong', 23: 'Hillview', 24: 'Lim Chu Kang',
      25: 'Kranji', 26: 'Upper Thomson', 27: 'Yishun', 28: 'Seletar'
    };
