...
export class SEOPageGenerator {
  ...
  static async generatePageContent(
    ...
    try {
      ...
      const content: SEOPageContent = {
        intro_text: await this.generateIntroText(pageType, filters, businesses.length),
  ...
  private static async getFilteredBusinesses(filters: Record<string, any>): Promise<Business[]> {
    let query = supabase
  ...
  private static calculateBusinessStats(businesses: Business[]) {
    ...
    const avgRating = businesses.reduce((sum, b) => sum + b.rating, 0) / totalCount;
    
    // Price distribution
    const priceDistribution: Record<string, number> = {};
    businesses.forEach(b => {
      if (b.price_range) {
        priceDistribution[b.price_range] = (priceDistribution[b.price_range] || 0) + 1;
      }
    });
  ...
  private static async generateIntroText(
    ...
    return templates[pageType] || `Discover ${businessCount} halal businesses in Singapore with verified certification and authentic experiences.`;
  }

  // Generate highlights for the page
  private static generateHighlights(businesses: Business[], filters: Record<string, any>): string[] {
    ...
    if (topRated.length > 0) {
      highlights.push(`Top-rated establishments include ${topRated.map(b => b.name).join(', ')}`);
    }

    // Premium businesses
    const premiumCount = businesses.filter(b => b.is_premium).length;
    if (premiumCount > 0) {
      highlights.push(`${premiumCount} premium listings with enhanced features and verified information`);
    }
    ...
    // Delivery options
    const deliveryCount = businesses.filter(b => 
      b.delivery_platforms && b.delivery_platforms.length > 0
    ).length;
    if (deliveryCount > 0) {
  ...
  private static async generateLocalInfo(filters: Record<string, any>): Promise<string | undefined> {
  ...
  static async generateAndStorePage(
    ...
    try {
      ...
      const { title, metaDescription, h1Title } = this.generateSEOMetadata(pageType, filters, content.business_stats.total_count);

      // Generate schema markup
      const schemaMarkup = this.generateSchemaMarkup(pageType, filters, content);
  ...
  static async generateAllPages(): Promise<void> {
    try {
      ...
      const pages: Array<{ slug: string; pageType: SEOPage['page_type']; filters: Record<string, any> }> = [];
      ...
      for (let i = 0; i < pages.length; i += batchSize) {
        ...
        console.log(`Generated SEO pages ${i + 1}-${Math.min(i + batchSize, pages.length)} of ${pages.length}`);
